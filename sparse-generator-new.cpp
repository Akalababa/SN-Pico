...
#include <stdio.h>
#include "d3-ndrtosn.h"
int main() 
{
	FILE *file;
	file = fopen("d3_NB.h", "w");
    if (file == NULL) 
	{
	...
	while(k < n)
	{
		for(i = 0;i < n;i++)
		{
			...
			if(sum == 0)         //i列和为0，意味着此时没有比ti更高优先级的迁移 
			{
				for(j = 0; j < k; j++)
				{
					if(i != v_per[j])  //判断当前vector里是否已有该迁移序号 
						continue;
					else
						break;
				}	
		...
		{
			for(j = 0; j < n; j++)
				r[v_per[l]][j] = 0;  //存入后将i行所有元素置为0 显示出下一级优先级
			l++;
		}
		l = k;	
	}
 	...
 	// 打印排序后的优先级
    fprintf(file, "static int v_per[%d] = {", n);
    for (int i = 0; i < n; i++)
    {
    	fprintf(file,"%d%s", v_per[i], (i == n-1) ? "};\n" : ", ");
	}
