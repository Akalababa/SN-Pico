...
void loop() {
    ...
    while (go) {
	    ...
	    for (int i = 0; i < n; i++) {
    		int row_idx = vector[i]; // 从 vector 获取当前行索引
    		if (y[row_idx] == 0) {
        		continue; // 如果该行的 y 值已经为 0，跳过这一行的处理
    		}
    
    // 获取当前行的列索引范围
    		int start_idx = trs[row_idx];
    		int end_idx = trs[row_idx + 1];

    // 遍历该行有效的列
    		for (int idx = start_idx; idx < end_idx; idx++) {
        		int col_idx = rs[idx][0]; // 获取当前列索引
        		y[col_idx] = 0; // 直接更新 y[col_idx]
    		}
		}
